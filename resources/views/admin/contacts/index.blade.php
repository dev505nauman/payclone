...
<div class="panel">
    <h3 class="panel-heading" style="display:block;">Contacts
      <a class="btn btn-primary pull-right" data-toggle="modal" data-target="#addcustomermodal">+Contact</a>
 ...
 <button class='btn btn-warning' id="editBtn" onclick="editUser();">Edit</button>&nbsp;<button class='btn btn-danger' id="deleteBtn" onclick="deleteUser();">Delete</button>&nbsp;<button class='btn btn-success' id="mergeBtn" onclick="mergeUser();">Merge</button><br><br>
  ...
  <tbody id="customerData">
                 ...
                 @if($users!=null)
                    ...
                    <tr id="user{{$user['id']}}">
                        ...
                        <td><a href="{{url('view-customer-contacts/'.$user['id'])}}">{{$user['fname']}} {{$user['lname']}}</a></td>
                        <td>{{$user['dob']}}</td>
                        <td>{{$user['email']}}</td>
                ...
                @if($merge!=null)
                            @foreach($merge as $mergee)
                                <tr id="user{{$mergee['id']}}">
                                    <td><input class="customCheckClass" type="checkbox" name="customCheck[]" onclick="checkboxesClick(this,1)" value="{{$mergee['id']}}" disabled></td>
                                    <td class="fname_disp"><a href="{{route('customer-detail',['id'=>$mergee['id']])}}">{{$mergee['fname']}} {{$mergee['lname']}}</a></td>
                                    <td class="dob_disp"><a href="{{route('customer-detail',['id'=>$mergee['id']])}}">{{$mergee['dob']}}</a></td>
                                    <td class="email_disp"><a href="{{route('customer-detail',['id'=>$mergee['id']])}}">{{$mergee['email']}}</a></td>
                                    <td class="email_disp"><a href="{{route('customer-detail',['id'=>$mergee['id']])}}">{{$mergee['companyName']}}</a></td>
                                    <td>Merged</td>
                                </tr>
                @endforeach
                @endif
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
       ...
       <form method="post" action="{{ route('delete-customer') }}" id="deleteForm">
...
<div id="merge-contact-modal" class="modal fade" role="dialog">
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
       ...
       <form method="post" action="{{route('merge-contact-save')}}">
           <input type="hidden" name="id1" id="mergeId1">
           <input type="hidden" name="id2" id="mergeId2">
           
          @csrf
      
        <div class="modal-body">
            <center><h3 style="margin:0px 0px 20px;">Select Values To Appear In A Table As Merged Contact</h3></center>
            <div class="row">
                <div class="col-md-6">
                    <label>First Name</label>
                    <select class="form-control" name="mergeFname" id="mergeFname">
                    </select>
                </div>
                <div class="col-md-6">
                    <label>Last Name</label>
                    <select id="mergeLname" name="mergeLname" class="form-control">
                    </select>
                </div>
            </div>
            <div class="row">
                <div class="col-md-6">
                    ...
                    <select class="form-control" name="mergeDob" id="mergeDob">
                    </select>
                </div>
                <div class="col-md-6">
                    <label>Email</label>
                    <select id="mergeEmail" name="mergeEmail" class="form-control">
                    </select>
                </div>
            </div>
         ...
         <div class="modal-footer">
             <p style="float:left;color:red;">The contacts once merged cannot be unmerged in future!</p>
        <button type="submit" class="btn btn-success">Yes</button>
        <button type="button" class="btn btn-danger" data-dismiss="modal">No</button>
      </div>
      </form>
...
function editUser()
  {
   ...
   function mergeUser(){
    ...
    $(".customCheckClass:checked").each(function(){
        mergeArray.push($(this).val());
    });

    $.ajax({
        type: 'GET',
        url: "{{route('merge-contact-data')}}",
         dataType:'json',
        data:{
          'id':mergeArray,
        },
        success:function(data){
            if(data['flag']==1){
                $('#message').text(data['data']).attr('class','alert alert-danger');
            }if(data['flag']==0){
                $('#message').text('').attr('class','');
                $('#mergeId1').val(data['data'][0]['id']);
                $('#mergeId2').val(data['data'][1]['id']);
           
                $('#mergeFname').append('<option value="'+data['data'][0]['fname']+'">'+data['data'][0]['fname']+'</option><option value="'+data['data'][1]['fname']+'">'+data['data'][1]['fname']+'</option>');
                $('#mergeLname').append('<option value="'+data['data'][0]['lname']+'">'+data['data'][0]['lname']+'</option><option value="'+data['data'][1]['lname']+'">'+data['data'][1]['lname']+'</option>');
                $('#mergeDob').append('<option value="'+data['data'][0]['dob']+'">'+data['data'][0]['dob']+'</option><option value="'+data['data'][1]['dob']+'">'+data['data'][1]['dob']+'</option>');
                $('#mergeEmail').append('<option value="'+data['data'][0]['email']+'">'+data['data'][0]['email']+'</option><option value="'+data['data'][1]['email']+'">'+data['data'][1]['email']+'</option>');
                
                $('#merge-contact-modal').modal('show');
            }
        }
      });
  }
