  ...
  <div class="tab-content">
    ...
    <div class="row">
     <div class="col-md-12">
         <div class="panel panelt">
           ...
           <p style="background-color:lightgray;color:white"><span style="margin-left:10px;">Today, {{date('d-M-Y')}}</span></p>
           <ul>
               @foreach($charge as $charges)
            <li>{{$charges->paymentType}} <p style="float:right;margin-right:5px;">{{$charges->created_at->format('H:i')}}</p><p style="float:right;padding-right:8px; color:red">${{$charges->amount}}</p></li>
            @endforeach
    ...
    <div class="panel panel-headline" style="margin-top:50px; width:50%; margin:50px auto 0px;">
     ...
     <div class="col-md-6">
          ...
          <div class="col-md-6 bars">
              <label for="processed_by">Location</label>
...
<!-- </div>-->
    ...
    <h2 class="col-md-12">Transaction details</h2>
                  ...
                  <div class="col-md-6 bars">
                      <select class="form-control" id="ach-location" name="ach-location">
                          @foreach($users as $user)
                              <option value="{{$user->id}}">{{$user->fname}} {{$user->lname}}</option>
                          @endforeach
                      </select>
    ...
    <div class="panel panel-headline" style="margin-top:50px; width:50%; margin:50px auto 0px;">
        <div class="panel-body">
              ...
              <div class="row">
    <h2 class="col-md-12">Transaction<small style="margin-top: 5px;color: red;float: right;font-size: 15px;"> This Option Will Be availabel Soon</small></h2>
                  <div class="col-md-12 bars">
                      <label for="amount">Amount</label>
...
<div class="modal fade" id="submit_payment" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog process-modal" id="mainStep">
...
<div class="modal fade" id="signModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog process-modal">
       ...
       <div class="container-fluid" style="width:90%;">
           <div class="modal-content">
               <div class="modal-header"  style="background-color:#41B314;">
                   <h5 class="modal-title text-center" style="color:white; font-size:30px; letter-spacing:5px;">Step 3</h5>
                   </div>
                   <div class="modal-body" style="margin-top:50px;">
                       <h3 class="lead">Payment successful! Thankyou for choosing payclone</h3>
                       <center><p>Signature Here</p>
                       <div class="wrapper">
...
function sumbitModal(){
   ...
   //  $('.modal-dialog #finalStep').children().hide();
            ...
            $.ajax({
                ...
                success: function (data) {
                  if(data['data'] == "success"){
                    setTimeout(function() {
                        $('.modal-dialog #mainStep').html(modalProcessing);
                      }, 3000);
                      
                      setTimeout(function() {
                         $('.modal-dialog #mainStep').html(modalProcessing2); 
                      }, 6000);
                      
                      setTimeout(function() {
                        $('#submit_payment').modal('hide');
                        $('#signModal').modal('show');
